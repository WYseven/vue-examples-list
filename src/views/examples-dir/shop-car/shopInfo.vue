<template>
  <div class="cart-bar clearfix">
      <div class="section-left">
          ...
          <span class="cart-total">共 <i>{{$store.getters['car/shopLength']}}</i> 件商品，已选择 <i>{{checkedLen}}</i> 件</span>
      </div>
      <span class="total-price">
          合计：<em>{{$store.getters['car/totalPrice']}}</em>元
      </span>
      <a href="javascript:void(0);" class="btn btn-a btn btn-primary" :class="{'btn-disabled': !checkedLen}">去结算</a>
      <div :class="{'no-select-tip': !checkedLen,hide: checkedLen}">
          请勾选需要结算的商品
...
export default {
  computed: {
    checkedLen () {
      return this.$store.getters['car/checkedLen']
    }
  }
}
</script>
